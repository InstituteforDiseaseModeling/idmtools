...
def build_image(username, password, no_cache=True):
    """
    ...
    Returns:
        None
    """
    #username = "username@idmod.org"
    #password = "password"

    version = get_latest_image_version_from_registry(username, password)
    if no_cache:
        cmd = ['docker', 'build', '--tag', f'{DOCKER_REPO}/{IMAGE_NAME}:{version}', '--no-cache', '-f', 'Dockerfile',
               f'{BASE_DIR}']
    else:
        cmd = ['docker', 'build', '--tag', f'{DOCKER_REPO}/{IMAGE_NAME}:{version}', '-f', 'Dockerfile', f'{BASE_DIR}']
...
if __name__ == "__main__":
    # Usage: python build_docker_image.py --username username --password password
    # To clear cache while building image, add "--no_cache True" in the end of above command
    ...
    parser.add_argument("--no_cache", default=False, help="docker build with no-cache")
    args = parser.parse_args()
    build_image(args.username, args.password, args.no_cache)
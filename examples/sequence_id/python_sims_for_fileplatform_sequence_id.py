"""
Simple python simulation running FilePlatform with user defined sequence ids for suite/experiment/simulation ids instead
of uuid string. The sequence id format is defined in idmtools.ini file.

Note, this example with FilePlatfrom does not actually execute simulations but just creates file structures for suite/
experiment/simulations. Although you can still execute simulations for this specific example by run "bash batch.sh"
under result's experiment dir. For any emodpy-disease workflows, you can either use SlurmPlatform or ProcessPlatform.

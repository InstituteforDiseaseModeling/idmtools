...
def map_item(item: IItem, get_item=None) -> Dict[Union[str, UUID], Dict]:
    """
    ...
    return get_item(item, analyzers, platform)
...
def _file_get_mapped_data_for_item(item: IEntity, analyzers: TAnalyzerList, platform: 'IPlatform') -> Dict[Union[str, UUID], Dict]:
    """
    Get mapped data from an item.
    ...
    try:
        ...
        # The byte_arrays will associate filename with content
        if len(filenames) > 0:
            file_data = platform.get_file_uris(item, filenames)
        else:
            file_data = dict()

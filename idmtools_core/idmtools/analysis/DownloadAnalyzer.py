...
class DownloadAnalyzer(IAnalyzer):
    ...
    def select_simulation_data(self, data, item):
...
if __name__ == '__main__':
    ...
    # I know, this is platform-specific. We need a general, platform-independent way of asking for items/simulations
    # that are contained one level down in a hierarchy.
    experiment = platform.retrieve_experiment(experiment_id=experiment_id)
    platform.restore_simulations(experiment=experiment)

    manager = AnalyzeManager(configuration={}, platform=platform, items=experiment.simulations, analyzers=analyzers)
    manager.analyze()

...
class TestLocalRunnerCLI(ITestWithPersistence):
    ...
    def test_local_runner_cli(self):
        ...
        command = ['python', '-m', 'idmtools_local.cli.run', 'simulation', 'status', '--experiment-id', str(self.pe.uid)]
        ...
        sims_from_simulation_CLI_sorted = sorted(sims_from_simulation_CLI, key=itemgetter('simulation_uid', 'experiment_id'))
        sims_from_experiment_CLI_sorted = sorted(sims_from_experiment_CLI, key=itemgetter('simulation_uid', 'experiment_id'))
        ...
        command = ['python', '-m', 'idmtools_local.cli.run', 'experiment', 'status', '--id', str(self.pe.uid), '--tag', 'idmtools', 'idmtools-automation']
        ...
        self.assertIn("{'idmtools': 'idmtools-automation', 'string_tag': 'test', 'number_tag': 123, 'type': 'idmtools_models.python.PythonExperiment'}", output_experiment)

...
sys.path.append(cwd)
from helper import remove_dir, verify_result
...
class TestFolder(unittest.TestCase):
    ...
    def test_2(self):
        ...
        experiment.run(platform=self.platform, wait_until_done=False, dry_run=True)
        self.verify_dir(f"{self.job_directory}/__EXPERIMENT__/test_2_{experiment.id}", experiment)

    # Test case to verify the experiment directory structure when experiment name has special character: :
    def test_3(self):
        experiment = Experiment.from_task(self.task, name='test:3')
        experiment.run(platform=self.platform, wait_until_done=False, dry_run=True)
        self.verify_dir(f"{self.job_directory}/__EXPERIMENT__/test_3_{experiment.id}", experiment)

    # Test case to verify the experiment directory structure when experiment name has special character: ?
    def test_4(self):
        experiment = Experiment.from_task(self.task, name='test?4')
        experiment.run(platform=self.platform, wait_until_done=False, dry_run=True)
        self.verify_dir(f"{self.job_directory}/__EXPERIMENT__/test_4_{experiment.id}", experiment)
    ...
    def test_5(self):
        ...
        experiment.run(platform=self.platform, wait_until_done=False, dry_run=True)
        self.verify_dir(f"{self.job_directory}/__EXPERIMENT__/test_5__{experiment.id}", experiment)

    # Test case to verify the experiment directory structure when experiment name has special characters: ()
    def test_6(self):
        experiment = Experiment.from_task(self.task, name='test(6)')
        experiment.run(platform=self.platform, wait_until_done=False, dry_run=True)
        self.verify_dir(f"{self.job_directory}/__EXPERIMENT__/test_6__{experiment.id}", experiment)

    # Test case to verify the experiment directory structure when experiment name has special characters: []
    def test_7(self):
        experiment = Experiment.from_task(self.task, name='test[7]')
        experiment.run(platform=self.platform, wait_until_done=False, dry_run=True)
        self.verify_dir(f"{self.job_directory}/__EXPERIMENT__/test_7__{experiment.id}", experiment)

    # Test case to verify the experiment directory structure when experiment name has special characters: *
    def test_8(self):
        experiment = Experiment.from_task(self.task, name='test*8')
        experiment.run(platform=self.platform, wait_until_done=False, dry_run=True)
        self.verify_dir(f"{self.job_directory}/__EXPERIMENT__/test_8_{experiment.id}", experiment)

    # Test case to verify the experiment directory structure when experiment name has special characters: ?
    def test_9(self):
        experiment = Experiment.from_task(self.task, name='test?9')
        experiment.run(platform=self.platform, wait_until_done=False, dry_run=True)
        self.verify_dir(f"{self.job_directory}/__EXPERIMENT__/test_9_{experiment.id}", experiment)

    # Test case to verify the experiment directory structure when experiment name has special characters: |
    def test_10(self):
        experiment = Experiment.from_task(self.task, name='test|10')
        experiment.run(platform=self.platform, wait_until_done=False, dry_run=True)
        self.verify_dir(f"{self.job_directory}/__EXPERIMENT__/test_10_{experiment.id}", experiment)

    # Test case to verify the experiment directory structure when experiment name has special characters: `
    def test_11(self):
        experiment = Experiment.from_task(self.task, name='test`11')
        experiment.run(platform=self.platform, wait_until_done=False, dry_run=True)
        self.verify_dir(f"{self.job_directory}/__EXPERIMENT__/test_11_{experiment.id}", experiment)

    # Test case to verify the experiment directory structure when experiment name has special characters: ,
    def test_12(self):
        experiment = Experiment.from_task(self.task, name='test,12')
        experiment.run(platform=self.platform, wait_until_done=False, dry_run=True)
        self.verify_dir(f"{self.job_directory}/__EXPERIMENT__/test_12_{experiment.id}", experiment)

    # Test case to verify the experiment directory structure when experiment name has special characters: $
    def test_13(self):
        experiment = Experiment.from_task(self.task, name='test$13')
        experiment.run(platform=self.platform, wait_until_done=False, dry_run=True)
        self.verify_dir(f"{self.job_directory}/__EXPERIMENT__/test_13_{experiment.id}", experiment)

    # Test case to verify the experiment directory structure when experiment name has special characters: !
    def test_14(self):
        experiment = Experiment.from_task(self.task, name='test!14')
        experiment.run(platform=self.platform, wait_until_done=False, dry_run=True)
        self.verify_dir(f"{self.job_directory}/__EXPERIMENT__/test_14_{experiment.id}", experiment)
    ...
    def test_15(self):
        ...
        experiment.run(platform=self.platform, wait_until_done=False, dry_run=True)
        self.verify_dir(f"{self.job_directory}/__EXPERIMENT__/test15__{experiment.id}", experiment)
    ...
    def test_suite(self):
        ...
        self.verify_dir(f"{self.job_directory}/__EXPERIMENT__/experiment+1_{experiment.id}", experiment)

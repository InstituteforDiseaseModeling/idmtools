...
@click.option('-c', '--container', help="Container ID")
@click.option('-i', '--index-url', help="index-url for pip install")
@click.option('-e', '--extra-index-url', help="extra-index-url for pip install")
def install(package: str = None, container: str = None, index_url: str = None, extra_index_url: str = None):
    """
    ...
    """
    if container is None:
        user_logger.warning("Container ID is required.")
        exit(-1)
    ...
    command = f'docker exec {container} bash -c "pip3 install {package}"'
    try:

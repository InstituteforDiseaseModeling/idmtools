...
class IterationManagerPythonTask(PythonTask):
    ...
    def generate_args_for_iteration(self, iteration_number, depends_on=None):
        all_args = {}
        ...
        for arg, arg_config in self.iteration_args['method_kwargs'].items():
            if 'value' in arg_config:
                # use an explicitly specified argument value
                method_kwargs[arg] = arg_config['value']
            else:
                # use an iteratively defined argument value
    ...
    def args_for_last_iteration(self):
        # convenience method for common pattern, doesn't include depends_on arguments
        iteration_numbers = self.discover_iteration_numbers()
        last_iteration_number = iteration_numbers[-1]
        args = self.generate_args_for_iteration(iteration_number=last_iteration_number)
        return args
    ...
    def make_new_iteration_task(self, iteration_number, depends_on):
        ...
        all_args = self.generate_args_for_iteration(iteration_number=iteration_number, depends_on=depends_on)
    ...
    def discover_iteration_numbers(self):
    ...
    def discover_additional_tasks(self, workflow=None):
        ...
        task_numbers = self.discover_iteration_numbers()
        if len(task_numbers) == 0:
            return  # nothing to do

        # Assumes sequentiality of task numbers!

...
class RunAnalyzeTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # TODO: these tests depend on EMOD experiments which will be disabled; to write tests for Python experiments
    ...
    def setUp(self):
        ...
        self.wi_name = "idmtools SSMT WorkItemManager Test"
    ...
    def test_ssmt_workitemmanager_user_files(self):
        ...
        # upload files to docker's current folder as linked file
    ...
    def test_ssmt_workitemmanager_asset_files(self):
        ...
        platform = Platform('COMPS2')
        wi = SSMTWorkItem(item_name=self.wi_name, command=command, asset_files=asset_files,
                          related_experiments=[self.exp_id], tags=self.tags)
        wim = WorkItemManager(wi, platform)
        wim.process(check_status=True)
        self.validate(wi)
    ...
    def test_ssmt_workitemmanager_with_input_folder(self):
        ...
        # upload run_PopulationAnalyzer1.py file to current dir in docker
        user_files = FileList()
    ...
    def test_ssmt_workitemmanager_with_output_folder(self):
        ...
        # upload run_PopulationAnalyzer_with_output_folder.py file to current dir in docker
        user_files = FileList()
    ...
    def test_ssmt_workitemmanager_with_multiple_analyzers(self):
        import pandas
        expid = self.exp_id
        command = "python run_2analyzers.py " + expid

        # add all files under 'analyzer' folder and put folder (relative_path) in docker
        user_files = FileList(root='.',
                              files_in_root=['PopulationAnalyzer.py', 'TimeseriesAnalyzer.py', 'run_2analyzers.py'])
